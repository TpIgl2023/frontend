...
export default function ModifyArticle({Article}) {
  ...
  const [title, setTitle] = useState(TestArticle.title); //
  const [summary, setSummary] = useState(TestArticle.resume); //
  const [content, setContent] = useState(TestArticle.text); //
  const [authors,setAuthors]=useState(TestArticle.authors)
  const [keywords,setKeyWords]=useState(TestArticle.keywords)
  const [referances,setReferances]=useState(TestArticle.references)
  const [institutions,setInstitutions]=useState(TestArticle.institutions)

  const [date,setDate] = useState(TestArticle.publishDate)
...
const addAuthor = (event) => {
  ...
  const removeInstitution = (indexToRemove) => {
    setInstitutions(prevInstitutions => prevInstitutions.filter((_, index) => index !== indexToRemove));
};
...
const addInstitution = (event) => {
  ...
  const removeKeyWord = (indexToRemove) => {
    setKeyWords(prevInstitutions => prevInstitutions.filter((_, index) => index !== indexToRemove));
};
...
const addKeyWord = (event) => {
  ...
  const removeReferance = (indexToRemove) => {
    setReferances(prevInstitutions => prevInstitutions.filter((_, index) => index !== indexToRemove));
};
    ...
    <>
        ...
        <form action="">
          <div className="w-[90%] sm:w-[80%] mx-auto"> 
            ...
            {/*Titre */}
            <label className="font-inter font-semibold text-xl sm:text-2xl md:text-3xl sm:pl-[50px] ">Titre de l'article : </label>
            <div className="my-[20px] p-6 border-main border-2">
              <textarea className=" w-full  font-inter text-xl  outline-none" name="summary" id="" cols="30" rows="2" onChange={handleTitleChange} value={title}/>
            </div>

            {/*Summary */}

            <div className="pt-10">
               <label className="font-inter font-semibold text-xl sm:text-2xl md:text-3xl sm:pl-[50px] py-10">Résumé de l'article : </label>
            </div>
            <div className="my-[20px] p-6 border-main border-2">
              <textarea className=" w-full  font-inter text-xl  outline-none" name="summary" id="" cols="30" rows="5" onChange={handleSummaryChange} value={summary}/>
            </div>
            {/*Authors */}
            <div className="pt-10">
               <label className="font-inter font-semibold text-xl sm:text-2xl md:text-3xl sm:pl-[50px] py-10">Auteurs de l'article : </label>
            </div>
            <div className="my-[20px] p-6 sm:px-10 border-main bg-main rounded-3xl border-2">
              <ul>
              {
                ...
                <div className=" mt-[10px] sm:pt-10 p-4 bg-main flex justify-center items-center gap-[10px] border-t-2 border-white">
                  <input type="text" name="authorName" value={newAuthor} onChange={handleAuthorChange} placeholder="Add new author" className="rounded-2xl px-4 py-2 *:bg-white font-inter outline-none w-[70%] sm:w-[50%]" />
                  <div onClick={addAuthor}>
                    <AddIcon fontSize='large' className="text-white cursor-pointer" />
                  </div>
                </div>
              </ul>
            </div>            
            {/*Institutions */}
            <div className="pt-10">
               <label className="font-inter font-semibold text-xl sm:text-2xl md:text-3xl sm:pl-[50px] py-10">Institutions de l'article : </label>
            </div>
            <div className="my-[20px] p-6 sm:px-10 border-main bg-main rounded-3xl border-2">
              <ul>
              {
                institutions.map((institution, index) => ( 
                  <ListItem author={institution} index={index} remove={removeInstitution} />
                ))}
                <div className=" mt-[10px] p-4 sm:pt-10 bg-main flex justify-center items-center gap-[10px] border-t-2 border-white">
                  <input type="text" name="authorName" value={newinstitution} onChange={handleInstitutionChange  } placeholder="Add new Institution" className="rounded-2xl px-4 py-2 *:bg-white font-inter outline-none w-[70%] sm:w-[50%]" />
            ...
            {/*Content */}
            <div className="pt-10">
               <label className="font-inter font-semibold text-xl sm:text-2xl md:text-3xl sm:pl-[50px] py-10">Contenu de l'article : </label>
            </div>
            <div className="my-[20px] p-6 border-main border-2">
              <textarea className=" w-full  font-inter text-xl  outline-none" name="summary" id="" cols="30" rows="13" value={content} onChange={handleContentChange}/>
            </div>      
            {/*Ref */}
            <div className="pt-10">
               <label className="font-inter font-semibold text-xl sm:text-2xl md:text-3xl sm:pl-[50px] py-10">Réference de l'article : </label>
            </div>
            <div className="my-[20px] p-6 sm:px-10 border-main bg-main rounded-3xl border-2">
              <ul>
              {
                referances.map((referance, index) => ( 
                  <ListItem author={referance} index={index} remove={removeReferance} />
                ))}
                <div className=" mt-[10px] p-4 sm:pt-10 bg-main flex justify-center items-center gap-[10px] border-t-2 border-white">
                  <input type="text" name="authorName" value={newReferance} onChange={handleReferanceChange  } placeholder="Add new Referance" className="rounded-2xl px-4 py-2 *:bg-white font-inter outline-none w-[70%] sm:w-[50%]" />
            ...
            <div className="py-6 sm:py-10 flex  flex-col items-center md:flex-row sm:justify-evenly">
               <label className="font-inter font-semibold text-xl sm:text-2xl md:text-3xl  lg:text-3xl px-[50px] py-10  ">Date de publication de l'article : </label>
               <input type="date" max="2024-02-28" name="date" value={date} onChange={handleDateChange} className="outline-none text-xl sm:text-2xl lg:text-3xl   font-inter font-semibold " />
            </div>
            {/*Key words */}
            <div className="pt-10">
               <label className="font-inter font-semibold text-xl sm:text-2xl md:text-3xl pl-[50px] py-10">Mots clés de l'article : </label>
            </div>
            <div className="my-[20px] p-6 sm:px-10 border-main bg-main rounded-3xl border-2">
              <ul>
              {
                keywords.map((keyword, index) => ( 
                  <ListItem author={keyword} index={index} remove={removeKeyWord} />
                ))}
                <div className=" mt-[10px] p-4 sm:pt-10 bg-main flex justify-center items-center gap-[10px] border-t-2 border-white">
                  <input type="text" name="authorName" value={newKeyWord} onChange={handleKeyWord  } placeholder="Add new Key word" className="rounded-2xl px-4 py-2 *:bg-white font-inter outline-none w-[70%] sm:w-[50%]" />
